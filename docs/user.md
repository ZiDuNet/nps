...
- `User`：登录账号、密码、状态、备注、到期时间、最大客户端数、用户级隧道上限，以及可轮换的用户 API Token 哈希。
...
管理员可以在用户列表中生成用户专属 API Token。Token 只显示一次，服务端不保存原文；普通用户使用该 Token 调用 API 时自动按当前用户过滤资源，不能读取其他用户或系统级数据。
