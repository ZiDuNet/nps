...
当前版本：**v1.1.19**。本版本基于 `yisier/nps` 上游同步，并保留本项目的独立用户体系、现代化 Web 管理面板和兼容迁移能力。
...
- Web 管理面板支持客户端、隧道、域名解析、用户、全局配置等管理能力；仪表盘可进入按当前账号授权范围筛选的实时资源拓扑页，提供浅色/深色主题、中文/英文切换、响应式布局和键盘可访问交互。资源拓扑默认每 60 秒刷新，可在页面上自定义 5–3600 秒的刷新间隔，设置仅保存在当前浏览器；管理员和普通用户均可生成独立的只读大屏快捷地址。
