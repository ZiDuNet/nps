...
| 指标 | 含义 |
| --- | --- |
| 入口流量 | 公网访问者经 NPS 发往内网目标的累计字节数（公网 → 内网） |
| 出口流量 | 内网目标经 NPS 返回访问者的累计字节数（内网 → 公网） |
| 当前带宽 | 当前采样窗口内的传输速率，适合观察趋势，不是计费级精确值 |
| 当前连接数 | 当前客户端名下正在处理的连接数 |
| 当前 TCP 连接 | 单条 TCP 隧道当前建立的连接数；仅在 TCP 隧道列表显示 |
...
| 带宽限制 | 客户端级双向总速率上限，单位为 KiB/s；同一客户端下所有隧道共享一个令牌桶，允许短暂突发，不是单隧道或严格瞬时上限。需要 `allow_rate_limit=true`。 |
...
升级到 v1.1.12 时，旧版本中“两个方向同时累加”的历史流量会归并为“历史”总量，用于保留已消耗的流量配额；这部分无法倒推出真实方向，因此不会伪装成入口或出口。
...
| 隧道 | 运行中、已停止、等待客户端连接、启动失败 | 列表的“配置状态”表示规则是否启用；“监听状态”表示服务端是否实际持有监听。等待连接表示规则已启用但 NPC 尚未建立控制连接；启动失败会在展开行显示最后一次错误，例如端口冲突。 |
...
资源状态图按运行中、已停止、等待连接和启动失败汇总，不会把协议类型误当作运行状态。流量、连接和配额数值仍以服务端数据层为准，页面刷新失败时不会覆盖上一次成功的结果。
