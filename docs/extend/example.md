# 使用示例
## 统一准备工作（必做）
- 开启服务端，假设公网服务器 IP 为 1.1.1.1，配置文件中 `bridge_port` 为 8024，配置文件中 `web_port` 为 8081
- 访问 1.1.1.1:8081
...
**注意：域名解析模式为http反向代理，不是dns服务器，在web上能够轻松灵活配置。** 多用户共用管理员提供的泛解析后缀、证书续期和路由诊断请阅读[平台泛域名、证书热更新与规则诊断](platform-domain.md)。
...
**HTTPS：** 如需使用 HTTPS，请参阅[服务端增强功能](../server/nps_extend.md#使用-https)。
...
- 如果需要账号密码，进入该客户端的编辑页面，在「认证配置」里填写「Basic 认证用户名」和「Basic 认证密码」。SOCKS5 隧道页面本身不单独设置账号密码。
- 在外网环境的本机配置 socks5 代理（例如使用 Proxifier 进行全局代理），IP 为公网服务器 IP（1.1.1.1），端口为填写的监听端口（8003）。如果客户端配置了 Basic 认证，也要在代理客户端中填写同一组账号密码。

**注意**
如果客户端未配置 Basic 认证，SOCKS5 会允许无认证连接。公网开放 SOCKS5 时，建议务必设置账号密码，并配合防火墙或黑名单限制访问来源。
...
**适用范围：** 大流量传输场景。流量不经过公网服务器，但 P2P 穿透与 NAT 类型关系较大，不保证 100% 成功；可先执行[NAT 类型检测](../client/npc_extend.md#nat-类型检测)。
