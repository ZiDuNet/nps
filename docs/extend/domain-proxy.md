...
- **路径重写**：按行填写 `旧路径 => 新路径`，例如 `/api => /v1/api`；只改发往后端的路径，查询参数保持不变。
- **响应 Header 修改**：按行填写 `名称: 值`，或用 `-名称` 删除响应头。`Content-Length`、`Transfer-Encoding` 和 `Connection` 由 NPS 管理，不能被规则覆盖，以避免响应截断。
- **自动 CORS**：向响应追加通用跨域 Header，适合内网 API 被浏览器直接调用的场景。
- **兼容模式**：使用标准 HTTP 反向代理链路，适合 SSE、流式响应、复杂 Header 和 WebSocket；默认链路保留轻量的原始 HTTP 转发。
- **重定向地址**：填写完整 URL 后该规则返回 `307`，`{path}` 会替换为原始请求路径和查询参数。
