...
NPS 的域名代理按 HTTP 请求中的 `Host` 和可选的 URL 路径匹配规则。DNS 只负责把域名解析到 NPS 公网地址；请求到达 NPS 后才由 Host 规则决定转发目标。
...
域名规则可直接粘贴证书/私钥，或填写 NPS 机器上的证书文件路径。上传证书时 NPS 终结 TLS，后端可以读取 HTTP Header；未上传证书时由内网服务处理 TLS，NPS 只转发加密流量。
...
1. 检查启动日志是否有 `start http listener, port is <端口>`。
2. 从 NPS 机器本地带 Host 头验证：

   ```shell
   curl -i -H 'Host: app.example.com' http://127.0.0.1:30111/
   ```

3. 确认对应客户端在线、未到期，Host 规则已启用且目标地址从 NPC 网络可达。
4. 确认 DNS 指向 NPS 公网 IP，外部访问 URL 包含非标准端口。
5. 检查云安全组、系统防火墙、Docker 端口发布和前置反向代理。
