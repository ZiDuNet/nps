...
源码默认配置以 `conf/nps.conf` 为唯一来源；服务端会将它嵌入可执行文件，并在首次启动时释放到配置目录：
...
Linux 上请使用 `sudo` 执行安装。服务会固定引用系统安装路径（通常为 `/usr/bin/npc`，无权限时为 `/usr/local/bin/npc`），因此安装完成后可以移动或删除下载包目录。
