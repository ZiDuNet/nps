...
基于原版 nps 0.26.10 二次开发，修复了大量 bug，优化了性能和安全性，并重新设计了 Web UI。v1.1.1 起支持独立用户体系：管理员可创建用户并将多个客户端分配给同一个用户；普通用户也可在管理员配置的客户端配额内自行创建客户端。用户与客户端到期时间独立保存，客户端到期不得晚于用户到期，客户端留空时跟随用户。
